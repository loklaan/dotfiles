...
Prefer `Effect.gen` when writing Effects. It provides an async/await style that
...
**Critical**: Inside of Effects, use `Effect.try` or `Effect.tryPromise` instead
of `try / catch`.
...
Services represent a collection of related Effect functions that can be composed
together and reused across your application.
...
trace the execution of your code, log important events, and monitor
performance.
...
Use `vitest` to test your Effect code. Use the `@effect/vitest` package to
integrate Effect with Vitest.
...
- `McpServer`, `Toolkit` etc modules from `@effect/ai`: Write AI applications using
  Effect. Search for the `@effect/ai` README with the `effect_docs_search` MCP
  tool for more information.
